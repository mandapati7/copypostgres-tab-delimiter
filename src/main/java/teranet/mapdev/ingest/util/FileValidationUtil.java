...
public class FileValidationUtil {
     ...
     * 
     * @param file the file to validate
     * @param allowedExtensions allowed file extensions (without dot, e.g., "csv", "zip")
    ...
    public static void validateFileExtension(MultipartFile file, String... allowedExtensions) {
        ...
        boolean isValid = Arrays.stream(allowedExtensions)
                .anyMatch(ext -> lowerCaseFilename.endsWith("." + ext.toLowerCase()));
        
        if (!isValid) {
            String allowedTypes = String.join(", ", allowedExtensions).toUpperCase();
            throw new IllegalArgumentException(
                String.format("Invalid file type. Expected %s file but received '%s'. Please upload a valid file with extension: %s",
                    allowedTypes, filename, allowedTypes)
            );
        }
    }
    
    /**
     * Comprehensive file validation: checks if file is not empty and has correct extension.
     * 
     * @param file the file to validate
     * @param allowedExtensions allowed file extensions (without dot, e.g., "csv", "zip")
    ...
    /**
     * Validates multiple files: checks if all files are not empty and have correct extension.
     * 
    ...
    /**
     * Validates multiple files from a List: checks if all files are not empty and have correct extension.
     * 
