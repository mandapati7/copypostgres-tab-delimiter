...
public class DelimitedFileController {
     ...
     * 
     * Examples:
     * - Upload TSV with routing: POST ?format=tsv&hasHeaders=false&routeByFilename=true
     * - Upload CSV to staging: POST ?format=csv&hasHeaders=true&routeByFilename=false
     */
    @PostMapping(value = "/upload", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(
        summary = "Upload delimited file (CSV/TSV)",
        description = """
    ...
    public ResponseEntity<?> uploadDelimitedFile(
            ...
            @Parameter(description = "File format: csv or tsv (default: csv)")
            @RequestParam(value = "format", required = false) String format,
            
            @Parameter(description = "Does file have headers? (default: true for CSV, false for TSV)")
            @RequestParam(value = "hasHeaders", required = false) Boolean hasHeaders,
            
            @Parameter(description = "Route by filename? (default: from config)")
            @RequestParam(value = "routeByFilename", required = false) Boolean routeByFilename) {
    ...
    private String determineFormat(MultipartFile file, String requestedFormat) {
        ...
        if (ingestConfig.isInferFormatFromExtension()) {
        ...
        return ingestConfig.getDefaultFormat();
    }
    ...
    private void validateFile(MultipartFile file, String format) {
        // Basic validation
        FileValidationUtil.validateFile(file, format);
